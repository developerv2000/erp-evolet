<x-tables.template.main-template class="prd-invoices-table" :records="$records">
    {{-- thead titles --}}
    <x-slot:thead-rows>
        <tr>
            ...
            @foreach ($visibleTableColumns as $column)
                <th width="{{ $column['width'] }}">
                    @include('PRD.invoices.table.thead-columns')
                </th>
            @endforeach
        </tr>
    </x-slot:thead-rows>
        ...
        @foreach ($records as $record)
            <tr>
                ...
                @if ($trashedRecords)
                    ...
                    <td><x-tables.partials.td.restore :form-action="route('prd.invoices.restore')" :record-id="$record->id" /></td>
                @endif

                @foreach ($visibleTableColumns as $column)
                    <td>
                        @include('PRD.invoices.table.tbody-row-columns')
                    </td>
                @endforeach
            </tr>
        @endforeach
