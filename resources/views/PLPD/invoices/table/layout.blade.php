        ...
        <tr>
            ...
            @foreach ($visibleTableColumns as $column)
                <th width="{{ $column['width'] }}">
                    @include('PLPD.invoices.table.thead-columns')
                </th>
            @endforeach
        </tr>
    </x-slot:thead-rows>
        ...
        @foreach ($records as $record)
            <tr>
                ...
                @if ($trashedRecords)
                    ...
                    <td><x-tables.partials.td.restore :form-action="route('plpd.invoices.restore')" :record-id="$record->id" /></td>
                @endif

                @foreach ($visibleTableColumns as $column)
                    <td>
                        @include('PLPD.invoices.table.tbody-row-columns')
                    </td>
                @endforeach
            </tr>
        @endforeach
