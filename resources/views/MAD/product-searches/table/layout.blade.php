        ...
        <tr>
            ...
            @foreach ($visibleTableColumns as $column)
                <th width="{{ $column['width'] }}">
                    @include('MAD.product-searches.table.thead-columns')
                </th>
            @endforeach
        </tr>
    </x-slot:thead-rows>
        ...
        @foreach ($records as $record)
            <tr>
                ...
                @if ($trashedRecords)
                    ...
                    <td><x-tables.partials.td.restore :form-action="route('mad.product-searches.restore')" :record-id="$record->id" /></td>
                @endif

                @foreach ($visibleTableColumns as $column)
                    <td>
                        @include('MAD.product-searches.table.tbody-row-columns')
                    </td>
                @endforeach
            </tr>
        @endforeach
