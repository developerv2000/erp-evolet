<x-tables.template.main-template class="warehouse-products-table" :records="$records">
    {{-- thead titles --}}
    <x-slot:thead-rows>
        <tr>
            ...
            @foreach ($visibleTableColumns as $column)
                <th width="{{ $column['width'] }}">
                    @include('warehouse.products.table.thead-columns')
                </th>
            @endforeach
        </tr>
    </x-slot:thead-rows>
        ...
        @foreach ($records as $record)
            <tr>
                ...
                @if ($trashedRecords)
                    ...
                    <td><x-tables.partials.td.restore :form-action="route('warehouse.products.restore')" :record-id="$record->id" /></td>
                @endif

                @foreach ($visibleTableColumns as $column)
                    <td>
                        @include('warehouse.products.table.tbody-row-columns')
                    </td>
                @endforeach
            </tr>
        @endforeach
